...
function evaluate(str) {
    var stack = tokenize(compile(str));
    var res = [];
    console.log(stack);
    for (let index = 0; index < stack.length; index++) {
        let element = String(stack[index]);
        if (isDigit(element) || isNumeric(element)) {
...
function clickHandler(event) {
    ...
    if(text == "=") {
        document.getElementsByTagName("span")[0].textContent = evaluate(document.getElementsByTagName("span")[0].textContent);
    } else if(text == "C") {
...
window.onload = function () {
    ...
    others.addEventListener("click",clickHandler);
}
